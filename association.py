...
pairwise_distance = lambda f1, f2, reduce_dim: \
    tf.reduce_sum(tf.square(tf.subtract(f1, f2)), reduce_dim)

hinge_loss = lambda dist_pos, dist_neg: \
    tf.reduce_mean(tf.maximum(dist_pos - dist_neg + FLAGS.margin, 0))

l2_norm = lambda x: \
    tf.nn.l2_normalize(x, 1, 1e-10)

normalize = lambda v: \
    v ** 2 / (np.sum(v ** 2, 1, keepdims=True))
...
def association_loss(features_cam, intra_anchors_n, cross_anchors_n, labels_cam):
    ...
    for i in range(FLAGS.num_cams):
        ...
        dist_pos_intra.append(pairwise_distance(
            features_cam[i], tf.gather(intra_anchors_n[i], labels_cam[i]), 1))
        ...
        dist_neg.append(association_ranking(
            features_cam[i], labels_cam[i], intra_anchors_n[i]))
...
def association_ranking(features_cam, labels_cam, intra_anchors_n):
    ...
    dist_all = pairwise_distance(
        features_cam[:, tf.newaxis], intra_anchors_n, 2)
...
def cyclic_ranking(intra_anchors_batch_n, same_anchors_n,
                   other_anchors_n, labels_cam, start_sign):
    # perform cyclic ranking to discover similar tracklets across cameras
    # (1) rank to the other camera
    dist = pairwise_distance(
        intra_anchors_batch_n[:, tf.newaxis], other_anchors_n, 2)
    ...
    dist = pairwise_distance(
        rank1_anchors[:, tf.newaxis], same_anchors_n, 2)

    with tf.device('/cpu:0'): # place ranking on cpu
        _, rank1 = tf.nn.top_k(-dist, k=1, sorted=True)

    # (3) consistency condition
    consistent = tf.cast(tf.equal(tf.cast(labels_cam, dtype=tf.int32),
                                  tf.squeeze(rank1, squeeze_dims=1)), tf.int32)
    consistent = tf.cast(consistent * tf.cast(start_sign, tf.int32), tf.bool)

    return consistent, rank1_anchors


def update_intra_anchor(intra_anchors, intra_anchors_n,
                        features_cam, labels_cam):
    # update intra-anchor for each camera
    for i in range(FLAGS.num_cams):
        # compute the difference between old anchors and the new given data
        diff = \
            tf.gather(intra_anchors_n[i], labels_cam[i]) - features_cam[i]
        # update the intra-anchors under each camera
        intra_anchors[i] = \
            tf.scatter_sub(intra_anchors[i], labels_cam[i], FLAGS.eta * diff)

    return intra_anchors


def update_cross_anchor(cross_anchors, intra_anchors_n,
                        intra_anchors_batch_n, labels_cam, start_sign):
    # update cross-anchor
    for i in range(FLAGS.num_cams):
        ...
        cyclic_ranking(intra_anchors_batch_n[i], intra_anchors_n[i],
                       other_anchors_n, labels_cam[i], start_sign)

        # if the consistency fufills, update by
        # merging using the best-matched anchors in anther camera
        update = tf.where(consistent,
                          (intra_anchors_batch_n[i] + rank1_anchors) / 2,
                          intra_anchors_batch_n[i])

        # update the associate centers under each camera
        cross_anchors[i] = \
            tf.scatter_update(cross_anchors[i], labels_cam[i], update)
...
def learning_graph(features, labels, cams, reuse_variables, start_sign):
    ...
    # compute two association losses
    final_loss = association_loss(
        features_cam, intra_anchors_n, cross_anchors_n, labels_cam)

    # update the intra-anchors
    intra_anchors = update_intra_anchor(
        intra_anchors, intra_anchors_n, features_cam, labels_cam)
    ...
    cross_anchors = update_cross_anchor(
        cross_anchors, intra_anchors_n, intra_anchors_batch_n, labels_cam, start_sign)
